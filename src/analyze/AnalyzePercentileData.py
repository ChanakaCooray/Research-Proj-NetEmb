...
def generate_percentile_results(data_dir, metadata, bin_size, shift, output_dir):
    # create output directory if not exists
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    if shift == '0':
        output_file = os.path.join(output_dir, "percentile_info.csv")
